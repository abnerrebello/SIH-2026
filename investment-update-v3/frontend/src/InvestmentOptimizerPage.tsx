...
export default function InvestmentOptimizerPage() {
  ...
  const validationErrors = useMemo(() => {
    ...
    if (!Number.isFinite(engineers) || engineers < 1) errors.push("Security engineers must be at least 1.");
    if (!Number.isFinite(days) || days < 1) errors.push("Remediation window must be at least 1 day.");
  ...
  const inputsValid = validationErrors.length === 0;
  ...
  const constraintKey = `${budget}|${engineers}|${days}`;

  useEffect(() => {
    if (!inputsValid || mutation.isPending) return;
    if (lastScheduledKeyRef.current === constraintKey) return;

    const timer = window.setTimeout(() => {
      lastScheduledKeyRef.current = constraintKey;
      mutation.mutate();
    }, 1000);

    return () => window.clearTimeout(timer);
  }, [constraintKey, inputsValid, mutation.isPending]); // eslint-disable-line react-hooks/exhaustive-deps
      ...
      <section className="investment-workbench investment-workbench-v2">
        <div className="investment-workbench-header">
          <div>
            ...
            <div className="panel-subtitle">
              Define the maximum budget, available engineering capacity and time window the plan must respect.
            </div>
          </div>
        ...
        <div className="investment-input-grid investment-input-grid-v2">
          <ConstraintInput
            ...
            min={10000}
            hint="Maximum spend for the selected plan"
          ...
          <ConstraintInput
            ...
            min={1}
            hint="People available for remediation"
          />
          <ConstraintInput
            icon={<Clock3 size={18} />}
        ...
        <div className="investment-financial-strip investment-financial-strip-v2">
          <div className="investment-financial-main">
            ...
            <div className="investment-eal-flow">
              <strong>{formatCompactCurrency(result.current_eal)}</strong><ArrowRight size={18} /><strong>{formatCompactCurrency(result.optimized_eal)}</strong>
            </div>
            <small>Expected Annual Loss · modeled estimate</small>
          </div>
          <div className="investment-eal-avoided">
            <span>Exposure avoided</span>
            <strong>{formatCompactCurrency(result.financial_exposure_avoided)}</strong>
...
function ConstraintInput({
  ...
  const commit = () => {
    ...
    if (!cleaned || !Number.isFinite(parsed)) {
      setInputError(`Enter a value of at least ${formatInputMinimum(min)}.`);
      return;
    }
    if (parsed < min) {
