...
class InvestmentOptimizer:
    ...
    def optimize(
        ...
        if engineers < 2:
            raise ValueError(
                "Engineers must be at least 2."
            )

        if not math.isfinite(days) or days < 2:
            raise ValueError(
                "Time window must be at least 2 days."
            )
        ...
        # environments, every retained action is evaluated exactly.
        # Keeping this pool small bounds the combinatorial search and prevents
        # the API from becoming unresponsive as the environment grows.
        ...
        # Exhaustive search is bounded to at most 10 candidates and four
        # simultaneous actions. This keeps recomputation fast enough for an
        # interactive UI while retaining a diverse, AI-ranked candidate pool.
        best_actions: list[
            InvestmentAction
        ] = []
        ...
        max_portfolio_size = min(len(candidates), 4)
        ...
        return {
            "ai_model": {
                ...
                "search_mode": "bounded-diverse-exact",
            },
            "current_risk": round(
                current_risk,
                2,
    ...
    def _empty_result(
        ...
        return {
            ...
            # Keep the response schema complete even when no action is feasible.
            # The frontend expects these financial fields to exist.
            "current_eal": 0.0,
            "optimized_eal": 0.0,
            "financial_exposure_avoided": 0.0,
            "rosi": 0.0,
            "actions": [],
            "alternatives": [
                {
